...
| # | 모듈명 | 배우는 기능 | 레퍼런스 파일 | 해금 조건 |
|---|--------|-----------|--------------|-----------|
| 6 | Plugin + Clarify | 플러그인, Clarify 3모드 | `references/module-06-clarify.md` | 모듈 5-4 완료 (예외 허용) |
| 7 | Skill Creator | 스킬 제작, Eval, A/B | `references/module-07-skill-creator.md` | 모듈 5-4 완료 (예외 허용) |
| 8 | Agent Teams | AGENTS.md, TF 파이프라인 | `references/module-08-agent-teams.md` | 모듈 5-4 완료 (예외 허용) |
| 9 | 팀 전파 + GitHub | 배포, 온보딩, Session Wrap | `references/module-09-github.md` | 모듈 5-4 완료 (예외 허용) |
| 10 | 종합 실전 프로젝트 | M1~M9 통합 | `references/module-10-capstone.md` | 모듈 5-4 완료 (예외 허용) |
...
- 모듈 1은 항상 열려 있음
- **M5 서브모듈:** 5-1 → 5-2 → 5-3 → 5-4 순차 해금. M5-4 완료 = M5 전체 완료
...
```json
{
  ...
  "modules": {
    ...
    "5-1": { "status": "completed|in_progress|locked", "quiz_score": 0, "quiz_total": 0, "last_step": "string", "weekly_mission": "not_started|completed" },
    "5-2": { "status": "completed|in_progress|locked", "quiz_score": 0, "quiz_total": 0, "last_step": "string", "weekly_mission": "not_started|completed" },
    "5-3": { "status": "completed|in_progress|locked", "quiz_score": 0, "quiz_total": 0, "last_step": "string", "weekly_mission": "not_started|completed" },
    "5-4": { "status": "completed|in_progress|locked", "quiz_score": 0, "quiz_total": 0, "last_step": "string", "weekly_mission": "not_started|completed" },
...
### "목차" / "menu" 감지 시
    ...
    🔒 4. Subagent
    🔒 5-1. MCP 개념 + 설치
    🔒 5-2. Google Sheets 조회+입력
    🔒 5-3. Forms+Sheets+Gmail
    🔒 5-4. Slack+세일즈맵+Context Sync
