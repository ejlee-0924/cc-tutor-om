...
📋 아래를 복사해서 입력하세요:
    ...
    3. 불필요한 파일이 있는지 확인해줘
    4. git init하고 첫 커밋을 만들어줘
    5. GitHub에 push하는 방법을 단계별로 알려줘

README 정비 → .gitignore 확인 → git init → commit → push 방법까지 한 흐름으로 진행한다.
(GitHub 계정이 없거나 권한이 없으면 방법만 이해하고 넘어감)

---

### Step 3 — Session Wrap 체험
