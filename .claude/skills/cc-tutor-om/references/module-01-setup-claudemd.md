...
### Step 5 — CLAUDE.md 개념 이해

📋 아래를 복사해서 입력하세요:

    CLAUDE.md가 뭔지 설명해줘

프로젝트 레벨 vs 개인 레벨의 차이를 확인한다.

---

### Step 6 — OM팀 CLAUDE.md 함께 작성

📋 아래를 복사해서 입력하세요:

    이 프로젝트의 CLAUDE.md를 만들어줘. 아래 내용을 포함해서:

    1. 항상 한국어로 응답
    2. 교안 검수 시 박규은 프로토콜 적용:
       - REJECT: Fatal Error 1건 이상 (내용 오류, 기업명 오타, 누락 섹션)
       - CONDITIONAL: Fatal 0건, Quality Issue 1~3건
       - PASS: 수정 불필요
    3. 만족도 분석 시 원문 반드시 보존, 긍정/부정 분류
    4. 강사 피드백 메일은 감사 인사로 시작, 존칭 사용
    5. 고객사 보고 메일은 격식체, 데이터 근거 포함
    6. 파일 저장 시 reports/ 폴더에 날짜 prefix

생성된 CLAUDE.md를 확인한다.

---

### Step 7 — CLAUDE.md 효과 체험

📋 아래를 복사해서 입력하세요:

    samples/survey-sample.csv를 분석해줘

CLAUDE.md에 적은 규칙(한국어 응답, 원문 보존, 긍정/부정 분류)이 자동으로 적용되는 것을 확인한다.
...
**Q3.** 프로젝트 CLAUDE.md와 개인 CLAUDE.md의 차이는?

- A) 프로젝트는 이 프로젝트에서만, 개인은 모든 프로젝트에서 적용
- B) 프로젝트가 더 우선순위가 높다
- C) 개인은 팀원에게 공유되지 않는다
- D) A와 C 모두 맞다

**정답: D**
> 프로젝트 CLAUDE.md는 팀 규칙(Git 공유 가능), 개인 CLAUDE.md는 내 선호(나만 적용). 둘 다 동시에 사용 가능합니다.
